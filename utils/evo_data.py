...
def load(model, target, device):
    preds = get_predictions(device, model)
    prices, other = supplementary_data(target)
    inputs, prices = stack_and_trim(preds, prices, other)
...
def get_data(device, model=params.BEST_MODEL_PATH, target=params.TARGET):
    inputs, prices = load(model, target, device)
    in_train, in_test, p_train, p_test = split_data(inputs, prices)
